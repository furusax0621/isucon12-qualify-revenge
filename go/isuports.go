...
type PlayerScoreRowForRanking struct {
	TenantID      int64  `db:"tenant_id"`
	ID            string `db:"id"`
	PlayerID      string `db:"player_id"`
	DisplayName   string `db:"display_name"`
...
func createBillingReport(ctx context.Context, tenantID int64, competitionID string) error {
	...
	query := "INSERT INTO `billing_report` ( " +
		"`tenant_id`, " +
		"`competition_id`, " +
		"`competition_title`, " +
		"`player_count`, " +
	...
	_, err = adminDB.ExecContext(ctx, query,
		...
		report.PlayerCount,
		report.VisitorCount,
		report.BillingPlayerYen,
		report.BillingVisitorYen,
		report.BillingYen,
	)
	return err
}
...
func competitionRankingHandler(c echo.Context) error {
	...
	// }
	// defer fl.Close()
	pss := []PlayerScoreRowForRanking{}
	if err := tenantDB.SelectContext(
		ctx,
		&pss,
		"SELECT `ps`.*, `p`.`display_name` AS `display_name` FROM `player_score` AS `ps`"+
